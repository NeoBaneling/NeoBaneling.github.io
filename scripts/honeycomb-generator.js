...
function drawHexagon(progress, x, y, side)
{
    ...
    ctx.beginPath();
    ctx.strokeStyle = "rgba(188,144,64,(canvas.width/2-x)/canvas.width/2)";
    ctx.lineWidth = 0.5;
    ...
    */
    ...
    if (Math.floor(Date.now() / 1000) % 10 > 2 && progress >= 4/6 - 0.002 && progress <= 4/6 + 0.002 && withinCanvas(x, y, length))
    {
